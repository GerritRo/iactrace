...
from .paths import match_path, path_to_string

FilterFn = Callable[[tuple[Any, ...], Any], bool]
PathSpec = str | Sequence[str] | FilterFn


def make_filter(
    model: Any,
    trainable: PathSpec | None = None,
    frozen: PathSpec | None = None,
) -> Any:
    """
    ...
    if trainable is not None:
        ...
        # frozen is guaranteed to be non-None here (either user-provided or set to [] above)
        assert frozen is not None
        filter_fn = _make_filter_fn(frozen, default=True, invert=True)

    def leaf_filter(path: tuple[Any, ...], leaf: Any) -> bool:
        if not eqx.is_array(leaf):
            return False
        return filter_fn(path, leaf)

    return jax.tree_util.tree_map_with_path(leaf_filter, model, is_leaf=eqx.is_array)


def _make_filter_fn(
    spec: PathSpec, default: bool, invert: bool = False
    ...
    if callable(spec):
        ...
        def fn(path: tuple[Any, ...], leaf: Any) -> bool:
...
def trainable_params(model: Any, spec: PathSpec) -> Any:
    """Get only trainable parameters."""
    return partition(model, trainable=spec)[0]


def show_structure(model: Any) -> None:
    """Print model structure with paths - useful for debugging filter patterns."""

    def print_leaf(path: tuple[Any, ...], leaf: Any) -> Any:
        path_str = path_to_string(path)
