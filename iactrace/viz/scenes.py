...
def show_telescope(telescope, camera=None, *, trajectory=None, **kwargs):
    ...
    telescope
        Telescope object
    camera
        Optional Camera object (for rendering sensors)
    trajectory
        Optional recorded ray paths to draw as polylines, in the
        ...
        tracer. Drawn via add_trajectories.
    **kwargs
        Additional options:
...
def show_camera(
    ...
    **kwargs,
    ...
    show_sensor_chain draws for one pixel (concentrator walls,
    ...
    it), use show_sensor_chain. For a camera image -- pixel values on
    the focal plane -- use iactrace.viz.show_image.
    ...
    ----------
    camera
        Camera object; every sensor group is drawn unless sensor_idx
        selects one.
    sensor_idx
        Optional index of a single sensor group to draw.
    trajectory
        Optional camera-frame ray paths to draw as polylines, as
        returned by Camera.trace. As elsewhere, this function draws
        what it is given and never runs the tracer itself.
    clip
        How to trim those paths. True (default) fits a box around the
    ...
    clip_margin
        Padding of the automatic box, as a fraction of the camera's
    ...
    entrances
        Also draw each pixel's entrance-aperture face (default
        False -- with thousands of pixels the filled faces hide the
        cones behind them).
    detail
        Tessellation scale, 1.0 being the full per-pixel detail
        show_sensor_chain uses. Defaults to 0.5: the whole-camera
    ...
    **kwargs
        cone_color / detector_color / sensor_color /
...
def show_sensor_chain(camera, sensor_idx=0, *, trajectory=None, **kwargs):
    ...
    concentrator is present and exposes Concentrator.cross_sections), and
    the photodetector's sensor surface -- its actual photocathode geometry, drawn
    curved when the photodetector owns a curved
    DetectionSurface, otherwise a
    flat active-area polygon at chain.detector_z.

    If the photodetector exposes a 3D envelope (PhotoDetector.envelope, e.g.
    a PMT), its glass body is lofted around
    ...
    ----------
    camera
        Camera object (the selected sensor group and its chain are read).
    sensor_idx
        Which sensor group to take the pixel geometry and chain from.
    trajectory
        Optional recorded ray paths to draw as polylines, in the
        pixel-local frame: a (steps + 1, N, 3) array or any object exposing
        a trajectory attribute (e.g. a
        ChainTrace). Drawn via
        add_trajectories.
    **kwargs
        entrance_color / cone_color / detector_color /
...
def export_mesh(telescope, filename):
    ...
    telescope
        Telescope object
    filename
        Output path (.glb, .gltf, .stl, .ply, .obj)
    """
