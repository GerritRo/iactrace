...
class PolygonalCone(Concentrator):
    ...
    - Geometry. The facet plane normals (n_hats) and the drawable
      cross-sections depend only on the polygon (n_sides, orientation)
      and on the meridian samples the subclass provides via _meridian.
    - Wall tracing. to_surface runs the shared trace_chain
      bounce loop, which asks the cone for the raw nearest wall hit
      (_nearest_hit); the cavity clamp (nearest_hit), the
      mouth-aperture mask (in_mouth) and the off-wall reflection
      book-keeping (reflect_ray) are shared here. All tracer coordinates
      are the cone frame (exit z = 0, mouth z = length).
    - Wall response. Every bounce costs wall_reflectivity, evaluated
    ...
    def wall_reflectivity(self, cos_theta_i: Array, wavelength: Array) -> Array:
        ...
        The scalar reflectivity is the flat bulk value; an optional
        reflectivity_curve multiplies in the coating response,
        ...
        idx = jnp.zeros(cos_theta_i.shape, dtype=jnp.int32)
    ...
    def reflect_ray(
        ...
        origin is nudged just off the wall along the reflected ray.
        """
...
class ChainTrace(NamedTuple):
    """Result of trace_chain.

    Attributes
    ----------
    rays
        Terminated RayBundle; alive marks the rays that
...
class _Event(NamedTuple):
    """The nearest trace event, as seen by _hit_event.

    Attributes
    ----------
    o_wall, refl_d, seg_wall
        reflected ray and path increment
    cos_theta_i
        incidence cosine of the bounce
    p_stop, t_stop
        landing point and ray parameter, valid where is_stop.
    is_stop, is_wall
        which of the two happened; neither means the ray left
...
def trace_chain(
    ...
    walls is the wall-based concentrator itself (a PolygonalCone):
    it answers the raw nearest wall hit (PolygonalCone._nearest_hit) and
    ...
    absorbed). Pass record_trajectory=True to also collect the per-step
     ray positions as a Trajectory on
    ChainTrace.trajectory (diagnostics; memory scales with
