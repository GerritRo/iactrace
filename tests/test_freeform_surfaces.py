...
class TestFreeformSurface:
    ...
    def test_index_slicing(self):
        g0 = jnp.zeros((5, 5))
        g1 = jnp.ones((5, 5))
        surf = FreeformSurfaceGroup.from_extent(jnp.stack([g0, g1]), jnp.array([0.5, 0.3]), 0.5)
        elem = surf._index(1)
...
class TestFreeformRenderPipeline:
    ...
    def test_intersect_at_vmapped(self):
        ...
        group = mirror_group_with_surface(SumSurfaceGroup([asph, free]), radius=0.4)
        n_rays = 32
        ...
        ts, pts, _ = jax.vmap(lambda o, d: group.surface.intersect_at(0, o, d))(origins, directions)
